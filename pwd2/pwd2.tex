...
\begin{definition}
    ...
    \(M\) na początku na stosie ma \(f\) symboli `l'.
    Zaczyna w stanie \(q_0\).
    Gdy automat przeczyta `a', wkłada na stos \(a^d\) i pozostaje w stanie \(q_0\).
    Gdy przeczyta `b', wkłada na stos \(a^e\) i pozostaje w stanie \(q_0\).
    Gdy przeczyta `\$' przechodzi do stanu \(q_1\).

    W stanie \(q_1\) z każdą literką `a' lub `b' zdejmuje ze stosu jedno `l'.
    Gdy stos jest pusty, a słowo jeszcze nie zostało przeczytane w pełni, to przechodzi do śmietnika.
...
\begin{proof}
    ...
    to w momencie przeczytania `\$' na stosie będzie \(d(\#_{a}(l))+e(\#_{b}(l))+f\) symboli `l'.
    Po przeczytaniu `\$' automat przechodzi do stanu \(q_1\).
    W stanie \(q_1\) zdejmuje ze stosu symbol `l' za każdą przeczytaną literę.
    Zatem w momencie przeczytania \(r\) na stosie 0 symboli `l' oraz będziemy w stanie \(q_1\).
    Zatem \(w \in L(M)\).
\end{proof}
\begin{proof}
    `\(\supseteq\)'
    ...
    na początku na stosie mamy \(f\) symboli `l',
    za każdą literę `a' wkładamy \(d\) symboli `l',
    za każdą literę `b' wkładamy \(e\) symboli `l'.
...
Jeśli \(d < 0 \land e < 0\), to \(L_p\) jest językiem skończonym, bo
\(\left\{ x,y: 0 \leq p(x,y) \right\}\) jest skończony, bo d.d.d. \(x,y\)
\(dx + ey < f\) dla \(x,y \geq 0\).
Skoro \(L_p\) jest językiem skończonym, to również regularny, a zatem bezkontekstowy.
