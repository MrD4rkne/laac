 ...
 \begin{center}
     \begin{tikzpicture}[shorten >=1pt, node distance=2cm, auto]
        ...
        \node[state] (q_03) [above of=q_00] {$q_{0,3}$};
         ...
         \node[state] (q_n1) [right of=q_21] {$q_{n-1,1}$};
         \node[state, accepting] (q_n0) [right of=q_n1] {$q_{n,0}$};
        ...
        \path[->]
            ...
            (q_21) edge [dotted] node {a} (q_n1)
            (q_n0) edge node {a} (q_n3)
            
            (q_n1) edge node {a} (q_n0)
...
\begin{remark}
    Na diagramie brakuje `śmietnika', czyli wszytskie brakujące krawędzie z literą \(b\), np. z \(q_{0,3},q_{0,1},q_{1,1}, q_{n+1,1}\).
