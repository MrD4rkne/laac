...
\begin{proof}
    ...
    Wszystkie przejścia są poprawne, gdyż zgodnie z definicją zmiany stanów na tych samych pozycjach.
    ...
    Będziemy utrzymywać niezmiennik, że dla danego \(j\) istnieje bieg o prefixie stanów \( \left( q_{1,1}, q_{2,1}, 0 \right) \xrightarrow{a_1}
    \left( q_{1,2}, q_{2,2}, 1 \right) \xrightarrow{a_2} \ldots \xrightarrow{a_{j-1}} \left( q_{1,j}, q_{2,j}, \left[ j \equiv 1 \mod 2  \right] \right) \).
    Dla \(j = 0\) niezmiennik jest zachowany, bo \(\{1, 2, \ldots j\} = \emptyset\).
